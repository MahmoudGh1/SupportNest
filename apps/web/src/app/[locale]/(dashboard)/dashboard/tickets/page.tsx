...
import {
	QueueTab,
	useTicketsQueue,
} from "@/app/[locale]/(dashboard)/dashboard/tickets/hooks/useTicketsQueue";
...
export default function TicketsPage() {
	...
	// const [activeTab, setActiveTab] = useState<TicketStatus | "ALL" | "MINE">(
	// currentUserRole === "SUPPORT_AGENT" ? "MINE" : "ALL",
	// );
	...
	const {
		...
		setActiveTab,
		refetch,
	} = useTicketsQueue(currentUserId as string);

	const countForTab = (tab: QueueTab) => counts?.[tab] ?? 0;
	...
	// };
	useEffect(() => {
		setActiveTab(currentUserRole === "SUPPORT_AGENT" ? "mine" : "all");
	}, []); // role-based default, set once on mount

	const handleSelectTicket = (ticket: Ticket) => {
		// const next = selected?.id === ticket.id ? null : ticket;
		// setSelected(next);
		// if (next) setMobileView("detail");
		router.push(`/dashboard/dashboard/tickets/${ticket.id}`);
	};
