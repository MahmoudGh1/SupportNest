...
function ResponsiveStyles() {
    return (
        <style>{`
            ...
            .org-grid { display: grid; grid-template-columns: 1.8fr 0.8fr 0.8fr 1.2fr 1.6fr; gap: 12px; padding: 14px 20px; align-items: center; }
            .org-header-grid { display: grid; grid-template-columns: 1.8fr 0.8fr 0.8fr 1.2fr 1.6fr; gap: 12px; padding: 12px 20px; }
            @media (max-width: 992px) {
                .org-header-grid { display: none !important; }
                .org-grid { display: flex !important; flex-direction: column !important; align-items: flex-start !important; gap: 14px !important; padding: 20px !important; }
                .org-cell-label { display: inline-block !important; width: 90px; font-size: 11px; font-weight: 600; color: ${S.textMuted}; text-transform: uppercase; letter-spacing: .02em; }
                .org-actions-wrapper { width: 100% !important; justify-content: flex-start !important; margin-top: 6px; padding-top: 14px; border-top: 1px dashed ${S.border}; }
                .org-responsive-row { display: flex !important; align-items: center !important; width: 100%; }
            }
            @media (max-width: 480px) {
                .org-actions-wrapper { flex-wrap: wrap !important; }
                .org-actions-wrapper button { flex: 1 1 calc(50% - 8px); text-align: center; justify-content: center; }
            }
        `}</style>
    );
}
...
function Shimmer({ width = "100%", height = 16, radius = 6 }: { width?: string | number; height?: number; radius?: number }) {
    return <div style={{ width, height, borderRadius: radius, background: `linear-gradient(90deg, ${S.border} 25%, #f0eff8 50%, ${S.border} 75%)`, backgroundSize: "200% 100%", animation: "shimmer 1.4s infinite", flexShrink: 0 }} />;
}
...
function CreateOrgModal({ onClose, onCreated, plans }: { onClose: () => void; onCreated: (org: AdminOrganization) => void; plans: AdminPlan[] }) {
    ...
    const handleNameChange = (v: string) => { setName(v); if (!slug || slug === slugify(name)) setSlug(slugify(v)); };
    ...
    const handleCreate = async () => {
        ...
        setLoading(true);
        try {
            const org = await api.createAdminOrganization({ name: name.trim(), email: email.trim(), password: password.trim(), slug: slug.trim(), plan_id: planId || undefined });
            onCreated(org);
        } catch (err: any) {
    ...
    const fs = (hasError: boolean): React.CSSProperties => ({ width: "100%", boxSizing: "border-box", height: 40, padding: "0 12px", border: `1.5px solid ${hasError ? S.danger : S.border}`, borderRadius: 8, fontSize: 13, fontFamily: "inherit", color: S.dark, outline: "none", background: "var(--surface-elevated)", transition: "border-color .15s" });
    ...
    return (
        <div style={{ position: "fixed", inset: 0, background: "rgba(26,24,48,0.45)", zIndex: 200, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
            <div style={{ background: S.surface, borderRadius: 16, width: "100%", maxWidth: 460, padding: "1.5rem", maxHeight: "90vh", overflowY: "auto", boxShadow: "0 24px 80px rgba(0,0,0,0.18)" }}>
                ...
                {errors.form && <div style={{ background: S.dangerBg, color: S.danger, borderRadius: 8, padding: "10px 14px", fontSize: 13, marginBottom: 14 }}>{errors.form}</div>}
                <div style={{ marginBottom: 14 }}>
                    ...
                    <input value={name} onChange={e => handleNameChange(e.target.value)} placeholder="Acme Corp" style={fs(!!errors.name)} />
                ...
                <div style={{ marginBottom: 14 }}>
                    ...
                    <input value={email} onChange={e => setEmail(e.target.value)} type="email" placeholder="admin@acme.com" style={fs(!!errors.email)} />
                ...
                <div style={{ marginBottom: 14 }}>
                    ...
                    <input value={password} onChange={e => setPassword(e.target.value)} type="password" placeholder="••••••••" style={fs(!!errors.password)} />
                ...
                <div style={{ marginBottom: 14 }}>
                    ...
                    <input value={slug} onChange={e => setSlug(slugify(e.target.value))} placeholder="acme-corp" style={{ ...fs(!!errors.slug), fontFamily: "monospace" }} />
                    {errors.slug ? <p style={{ fontSize: 11, color: S.danger, margin: "4px 0 0" }}>{errors.slug}</p> : <p style={{ fontSize: 11, color: S.textMuted, margin: "4px 0 0" }}><Trans>URL-safe identifier — lowercase, hyphens only.</Trans></p>}
                </div>
                <div style={{ marginBottom: 22 }}>
                    ...
                    <select value={planId} onChange={e => setPlanId(e.target.value)} style={fs(false)}>
                        <option value="">{t`Select a plan (optional)`}</option>
                        {plans.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}
                    </select>
                </div>
                <div style={{ display: "flex", gap: 10, justifyContent: "flex-end" }}>
                    <button onClick={onClose} style={{ padding: "9px 18px", borderRadius: 8, border: `1.5px solid ${S.border}`, background: "transparent", color: S.textSecondary, fontSize: 13, fontWeight: 500, cursor: "pointer", fontFamily: "inherit" }}><Trans>Cancel</Trans></button>
...
function EditOrgModal({ org, onClose, onUpdated, plans }: { org: AdminOrganization; onClose: () => void; onUpdated: (org: AdminOrganization) => void; plans: AdminPlan[] }) {
    ...
    const handleUpdate = async () => {
        ...
        setLoading(true);
        try {
            const updated = await api.updateAdminOrganization(org.id, { name: name.trim(), email: email.trim(), plan_id: planId || undefined });
            onUpdated(updated);
        } catch (err: any) {
    ...
    return (
        <div style={{ position: "fixed", inset: 0, background: "rgba(26,24,48,0.45)", zIndex: 200, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
            <div style={{ background: S.surface, borderRadius: 16, width: "100%", maxWidth: 460, padding: "1.5rem", boxShadow: "0 24px 80px rgba(0,0,0,0.18)" }}>
                ...
                <div style={{ marginBottom: 14 }}>
                    <label style={{ display: "block", fontSize: 12, fontWeight: 500, color: S.dark, marginBottom: 5 }}><Trans>Organization name</Trans></label>
                    <input value={name} onChange={e => setName(e.target.value)} placeholder="Acme Corp" style={fs(!!errors.name)} />
                    {errors.name && <p style={{ fontSize: 11, color: S.danger, margin: "4px 0 0" }}>{errors.name}</p>}
                </div>
                <div style={{ marginBottom: 14 }}>
                    <label style={{ display: "block", fontSize: 12, fontWeight: 500, color: S.dark, marginBottom: 5 }}><Trans>Contact email</Trans></label>
                    <input value={email} onChange={e => setEmail(e.target.value)} type="email" placeholder="contact@acme.com" style={fs(!!errors.email)} />
                    {errors.email && <p style={{ fontSize: 11, color: S.danger, margin: "4px 0 0" }}>{errors.email}</p>}
                </div>
                <div style={{ marginBottom: 22 }}>
                    <label style={{ display: "block", fontSize: 12, fontWeight: 500, color: S.dark, marginBottom: 5 }}><Trans>Subscription Plan</Trans></label>
...
function DeleteConfirmModal({ org, onClose, onDeleted }: { org: AdminOrganization; onClose: () => void; onDeleted: () => void }) {
    ...
    const handleDelete = async () => {
        ...
        setLoading(true);
        try { await api.deleteAdminOrganization(org.id); onDeleted(); }
        catch (err: any) { alert(err.message ?? t`Failed to delete organization.`); }
        finally { setLoading(false); }
    };
    ...
    return (
        <div style={{ position: "fixed", inset: 0, background: "rgba(26,24,48,0.45)", zIndex: 200, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
            <div style={{ background: S.surface, borderRadius: 16, width: "100%", maxWidth: 420, padding: "1.5rem", boxShadow: "0 24px 80px rgba(0,0,0,0.18)" }}>
                ...
                <h3 style={{ margin: 0, fontSize: 17, fontWeight: 700, color: S.dark }}><Trans>Delete organization?</Trans></h3>
                <p style={{ margin: "8px 0 16px", fontSize: 13, color: S.textMuted, lineHeight: 1.5 }}>
                    <Trans>This will permanently delete <strong>{org.name}</strong> and all data. This action cannot be undone.</Trans>
                </p>
                <div style={{ marginBottom: 20 }}>
                    ...
                    <input value={confirmName} onChange={e => setConfirmName(e.target.value)} placeholder={org.name} style={{ width: "100%", boxSizing: "border-box", height: 40, padding: "0 12px", border: `1.5px solid ${S.border}`, borderRadius: 8, fontSize: 13, fontFamily: "inherit", color: S.dark, outline: "none", background: "var(--surface-elevated)" }} />
                </div>
                <div style={{ display: "flex", gap: 10 }}>
                    <button onClick={onClose} style={{ flex: 1, padding: "10px", borderRadius: 8, border: `1.5px solid ${S.border}`, background: "transparent", color: S.textSecondary, fontSize: 13, fontWeight: 600, cursor: "pointer", fontFamily: "inherit" }}><Trans>Cancel</Trans></button>
                    <button onClick={handleDelete} disabled={loading || confirmName !== org.name} style={{ flex: 1, padding: "10px", borderRadius: 8, border: "none", background: loading || confirmName !== org.name ? S.dangerBg : S.danger, color: loading || confirmName !== org.name ? S.danger : "#fff", fontSize: 13, fontWeight: 600, cursor: loading || confirmName !== org.name ? "not-allowed" : "pointer", fontFamily: "inherit" }}>
                        {loading ? <Trans>Deleting...</Trans> : <Trans>Delete</Trans>}
                    </button>
                </div>
            </div>
        </div>
...
function Toast({ msg, type, onClose }: { msg: string; type: "success" | "error"; onClose: () => void }) {
    ...
    return (
        <div style={{ position: "fixed", bottom: 24, right: 24, left: 24, zIndex: 999, maxWidth: 360, marginLeft: "auto", background: type === "success" ? S.greenBg : S.dangerBg, border: `1px solid ${type === "success" ? S.green : S.danger}`, color: type === "success" ? "#0F6E56" : S.danger, borderRadius: 10, padding: "12px 18px", fontSize: 13, fontWeight: 500, display: "flex", alignItems: "center", gap: 8, boxShadow: "0 4px 20px rgba(0,0,0,0.10)", animation: "slideUp .2s ease" }}>
...
export default function OrganizationsPage() {
    ...
    const fetchPlans = useCallback(async () => {
        try { const res = await api.getPlans(); setPlans(res.map(p => ({ id: p.id, name: p.name, price_monthly: p.priceMonthly }))); }
    ...
    const fetchOrgs = useCallback(async () => {
        ...
        try {
            const res = await api.getAdminOrganizations({ search: search || undefined, is_active: statusFilter === "" ? "" : statusFilter === "true", page, limit: PAGE_SIZE });
            setOrgs(res.data); setMeta(res.meta);
        } catch (err: any) {
    ...
    useEffect(() => { fetchOrgs(); fetchPlans(); }, [fetchOrgs, fetchPlans]);
    useEffect(() => { const timer = setTimeout(() => { setPage(1); setSearch(searchInput); }, 400); return () => clearTimeout(timer); }, [searchInput]);

    const handleCreated = (org: AdminOrganization) => { setShowCreate(false); setToast({ msg: t`${org.name} created.`, type: "success" }); fetchOrgs(); };
    const handleUpdated = (org: AdminOrganization) => { setEditingOrg(null); setToast({ msg: t`${org.name} updated.`, type: "success" }); fetchOrgs(); };
    const handleDeleted = () => { const name = deletingOrg?.name ?? ""; setDeletingOrg(null); setToast({ msg: t`${name} deleted.`, type: "success" }); if (selectedOrgId) setSelectedOrgId(null); fetchOrgs(); };

    const handleToggleStatus = async (org: AdminOrganization) => {
        setActionLoading(org.id);
        try {
            if (org.is_active) { await api.suspendAdminOrganization(org.id); setToast({ msg: t`${org.name} suspended.`, type: "success" }); }
            else { await api.activateAdminOrganization(org.id); setToast({ msg: t`${org.name} activated.`, type: "success" }); }
            setOrgs(prev => prev.map(o => o.id === org.id ? { ...o, is_active: !o.is_active } : o));
        } catch (err: any) { setToast({ msg: err.message ?? t`Action failed.`, type: "error" }); }
    ...
    if (user?.role !== "SUPER_ADMIN") {
        return <div style={{ padding: "2rem", textAlign: "center", color: S.danger, fontWeight: 600 }}><Trans>Access Denied</Trans></div>;
    }

    if (selectedOrgId) {
        return <div style={{ padding: "1rem" }}><OrganizationDetail organizationId={selectedOrgId} onClose={() => setSelectedOrgId(null)} /></div>;
    }
    ...
    return (
        <div style={{ padding: "1rem", minWidth: 0 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20, gap: 16, flexWrap: "wrap" }}>
                <div>
                    <p style={{ fontSize: 10, fontWeight: 600, color: S.textMuted, letterSpacing: ".06em", textTransform: "uppercase", margin: "0 0 4px" }}><Trans>Platform Administration</Trans></p>
                    <h1 style={{ fontSize: "calc(18px + 0.4vw)", fontWeight: 750, color: S.dark, margin: 0 }}><Trans>Organizations</Trans></h1>
                </div>
            ...
            <div style={{ display: "flex", gap: 12, marginBottom: 16, flexWrap: "wrap", alignItems: "center" }}>
                <div style={{ position: "relative", flex: "1 1 240px" }}>
                    ...
                    <input value={searchInput} onChange={e => setSearchInput(e.target.value)} placeholder={t`Search organizations…`} style={{ width: "100%", boxSizing: "border-box", height: 38, padding: "0 12px 0 36px", border: `1.5px solid ${S.border}`, borderRadius: 8, fontSize: 13, fontFamily: "inherit", color: S.dark, outline: "none", background: S.surface }} />
                </div>
                <div style={{ display: "flex", gap: 4, background: S.surface, border: `0.5px solid ${S.border}`, borderRadius: 8, padding: 4, overflowX: "auto" }}>
                    {([
                        { label: t`All`, value: "" as const },
                        { label: t`Active`, value: "true" as const },
                        { label: t`Suspended`, value: "false" as const },
                    ] as const).map(opt => (
                        <button key={opt.value} onClick={() => { setStatusFilter(opt.value); setPage(1); }} style={{ padding: "6px 12px", borderRadius: 6, border: "none", cursor: "pointer", fontFamily: "inherit", fontSize: 12, fontWeight: 500, background: statusFilter === opt.value ? S.purple : "transparent", color: statusFilter === opt.value ? "#fff" : S.textMuted, transition: "all .15s", whiteSpace: "nowrap" }}>
                            {opt.label}
                        </button>
                    ))}
                </div>
            ...
            {loading ? <TableSkeleton /> : error ? (
                ...
                <div style={{ background: S.surface, borderRadius: 12, border: `0.5px solid ${S.border}`, overflow: "hidden" }}>
                    <div className="org-header-grid" style={{ borderBottom: `0.5px solid ${S.border}`, fontSize: 11, fontWeight: 600, color: S.textMuted, letterSpacing: ".04em", textTransform: "uppercase" }}>
                        ...
                        <div><Trans>Created</Trans></div>
                        <div style={{ textAlign: "right" }}><Trans>Actions</Trans></div>
                    </div>

                    {orgs.map(org => (
                        <div key={org.id} onClick={() => setSelectedOrgId(org.id)} className="org-grid" style={{ borderBottom: `0.5px solid ${S.border}`, background: S.surface, transition: "background .15s", cursor: "pointer" }}>
                            ...
                            <div className="org-actions-wrapper" style={{ display: "flex", gap: 8, justifyContent: "flex-end" }} onClick={e => e.stopPropagation()}>
                                <button onClick={() => handleToggleStatus(org)} disabled={actionLoading === org.id} title={org.is_active ? t`Suspend organization` : t`Activate organization`} style={{ padding: "6px 10px", borderRadius: 6, border: `1.5px solid ${S.border}`, background: "transparent", color: org.is_active ? S.danger : S.green, fontSize: 12, fontWeight: 500, cursor: "pointer", fontFamily: "inherit" }}>
                                    {actionLoading === org.id ? "…" : org.is_active ? <Trans>Suspend</Trans> : <Trans>Activate</Trans>}
                                </button>
                                <button onClick={() => setEditingOrg(org)} title={t`Edit details`} style={{ padding: "6px 10px", borderRadius: 6, border: `1.5px solid ${S.border}`, background: "transparent", color: S.textSecondary, fontSize: 12, fontWeight: 500, cursor: "pointer", fontFamily: "inherit" }}><Trans>Edit</Trans></button>
                                <button onClick={() => setDeletingOrg(org)} title={t`Delete organization`} style={{ padding: "6px 10px", borderRadius: 6, border: "none", background: S.dangerBg, color: S.danger, fontSize: 12, fontWeight: 500, cursor: "pointer", fontFamily: "inherit" }}><Trans>Delete</Trans></button>
                            </div>
                        </div>
                    ))}
                    ...
                    {totalPages > 1 && (
                        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 20px", background: "var(--surface-muted, #fafafa)", gap: 12, flexWrap: "wrap" }}>
                            ...
                            <div style={{ display: "flex", gap: 6 }}>
                                <button disabled={page === 1} onClick={() => setPage(p => Math.max(p - 1, 1))} style={{ padding: "6px 12px", borderRadius: 6, border: `1.5px solid ${S.border}`, background: S.surface, color: page === 1 ? S.textMuted : S.textSecondary, fontSize: 12, fontWeight: 500, cursor: "pointer" }}><Trans>Prev</Trans></button>
                                <button disabled={page === totalPages} onClick={() => setPage(p => Math.min(p + 1, totalPages))} style={{ padding: "6px 12px", borderRadius: 6, border: `1.5px solid ${S.border}`, background: S.surface, color: page === totalPages ? S.textMuted : S.textSecondary, fontSize: 12, fontWeight: 500, cursor: "pointer" }}><Trans>Next</Trans></button>
                            </div>
                        </div>
                    )}
                </div>
