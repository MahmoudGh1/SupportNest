...
async function apiGetTickets(filters?: {
  status?: TicketStatus; page?: number; limit?: number
...
async function apiStart(id: string):   Promise<Ticket> {
  const res  = await fetch(`${BASE}/tickets/${id}/start`, { method: "PATCH", credentials: "include" })
  const json = await res.json()
...
function PriorityBadge({ p }: { p: TicketPriority }) {
  return (
    <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${PRIORITY_CLASSES[p]}`}>
      {PRIORITY_LABEL[p]}
    </span>
  )
}
...
function StatusBadge({ s }: { s: TicketStatus }) {
  return (
    <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${STATUS_CLASSES[s]}`}>
      {STATUS_LABEL[s]}
    </span>
  )
}
  ...
  return (
    <button
      ...
      className={`w-full text-left px-4 py-3.5 border-b border-[#e8e6f0] transition-all duration-150 hover:bg-[#f6f5fc] group ${
        selected ? "bg-[#f6f5fc] border-l-2 border-l-[#534AB7]" : "border-l-2 border-l-transparent"
      }`}
    >
      <div className="flex items-start justify-between gap-2 mb-1.5">
        ...
        <span className="text-[11px] text-[#64607a] shrink-0">{timeAgo(ticket.createdAt)}</span>
      </div>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-2xl p-7 w-[420px] max-w-[90vw]">
        <h3 className="text-base font-bold text-[#1a1830] mb-1">Resolve ticket</h3>
        <p className="text-[13px] text-[#64607a] mb-4">Add a resolution note (optional but recommended).</p>
        <textarea
          value={note}
        ...
        <div className="flex gap-2 mt-4 justify-end">
          <button onClick={onCancel} className="px-4 py-2 text-sm text-[#64607a] rounded-lg hover:bg-[#f6f5fc] transition-colors">
            Cancel
          </button>
          <button
...
function TicketDetail({
  ...
  const roleBubble = (role: Message["role"]) => {
    if (role === "customer")    return "bg-[#f6f5fc] text-[#1a1830] rounded-br-sm"
    ...
    <div className="flex flex-col h-full relative">
        ...
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <p className="text-[12px] text-[#64607a] mt-1">
              ...
              {ticket.assignedTo && (
                <> · Assigned to <strong>{ticket.assignedTo.firstName} {ticket.assignedTo.lastName}</strong></>
              )}
            </p>
          </div>
      ...
      <div className="flex-1 overflow-y-auto px-5 py-4 bg-[#fafafa] space-y-3 min-h-0">
        {msgLoading ? (
          ...
          <div className="text-center text-sm text-[#64607a] py-10">No messages yet.</div>
        ) : (
          messages.map(msg => (
            <div
              key={msg.id}
            ...
            >
              <div className="max-w-[75%]">
                <div className={`text-[10px] font-semibold mb-1 ${msg.role === "customer" ? "text-left text-[#64607a]" : "text-right text-[#64607a]"}`}>
                  {roleLabel(msg.role)}
                  {msg.tier && <span className="ml-1 opacity-60">({msg.tier})</span>}
                </div>
                <div className={`px-3.5 py-2.5 rounded-2xl text-[13px] leading-[1.6] ${roleBubble(msg.role)}`}>
                  {msg.content}
                </div>
                <div className={`text-[10px] text-[#64607a] mt-0.5 ${msg.role === "customer" ? "text-left" : "text-right"}`}>
                  {fmt(msg.createdAt)}
                </div>
              </div>
            </div>
      ...
      {ticket.status !== "RESOLVED" && (
        <div className="px-4 py-3 border-t border-[#e8e6f0] bg-white shrink-0">
          <p className="text-[11px] text-[#64607a] mb-1.5 italic">Reply functionality requires message-send endpoint from backend.</p>
...
export default function TicketsPage() {
  ...
  return (
    <div className="flex flex-col h-full bg-white">
      ...
      {/* Page header */}
      <div className="px-6 py-4 border-b border-[#e8e6f0] shrink-0">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-[17px] font-bold text-[#1a1830]">Ticket Inbox</h1>
            <p className="text-[13px] text-[#64607a] mt-0.5">
              {meta ? `${meta.total} total ticket${meta.total !== 1 ? "s" : ""}` : "Loading…"}
            </p>
          </div>
          <button
            onClick={() => fetchTickets(activeTab)}
            className="flex items-center gap-1.5 text-[13px] text-[#64607a] px-3 py-1.5 rounded-lg hover:bg-[#f6f5fc] transition-colors"
          >
          ...
          {TABS.map(tab => (
            ...
            >
              ...
              <span className={`text-[10px] px-1.5 py-0.5 rounded-full ${
                activeTab === tab.value ? "bg-white/20 text-white" : "bg-[#e8e6f0] text-[#64607a]"
              }`}>
      ...
      <div className="flex flex-1 min-h-0 overflow-hidden">
        ...
        {/* Left — ticket list */}
        <div className={`border-r border-[#e8e6f0] overflow-y-auto shrink-0 ${selected ? "w-[320px]" : "flex-1"}`}>
          {loading ? (
            ...
            <div className="px-5 py-8 text-center">
              ...
              <button onClick={() => fetchTickets(activeTab)} className="text-[13px] text-[#534AB7] font-semibold">
                Try again
              </button>
            </div>
          ) : tickets.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-20 text-center px-5">
              <div className="w-12 h-12 rounded-2xl bg-[#EEEDFE] flex items-center justify-center mb-3">
                <i className="ti ti-ticket text-2xl text-[#534AB7]" />
              </div>
              <p className="text-sm font-semibold text-[#1a1830]">No tickets</p>
              <p className="text-[12px] text-[#64607a] mt-1">
                {activeTab === "ALL" ? "No tickets have been created yet." : `No ${activeTab.toLowerCase().replace("_", " ")} tickets.`}
              </p>
            </div>
          ) : (
            tickets.map(ticket => (
        ...
        {selected ? (
          ...
          <div className="flex-1 flex flex-col items-center justify-center bg-[#fafafa] text-center px-6">
            <div className="w-14 h-14 rounded-2xl bg-[#EEEDFE] flex items-center justify-center mb-4">
              <i className="ti ti-ticket text-3xl text-[#534AB7]" />
            </div>
            <p className="text-sm font-semibold text-[#1a1830]">Select a ticket</p>
            <p className="text-[13px] text-[#64607a] mt-1.5 max-w-[240px]">
              Click any ticket on the left to view the conversation and take action.
            </p>
          </div>
        )}
      </div>
