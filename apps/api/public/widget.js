...
(function () {
	...
	function injectStyles() {
      ...
      :root {
        --sn-font: "Inter", -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif;
        --sn-accent: #5b4eff;
        --sn-accent-dark: #4a3de0;
        --sn-accent-light: #ede9ff;
        --sn-accent-mid: rgba(91,78,255,0.15);
        --sn-header-bg: #1a1f36;
        --sn-header-bg2: #0f1629;
        --sn-radius-lg: 20px;
        --sn-radius-md: 14px;
        --sn-radius-sm: 10px;
        --sn-shadow-panel: 0 24px 64px rgba(0,0,0,0.22), 0 8px 24px rgba(0,0,0,0.1), 0 0 0 1px rgba(255,255,255,0.06);
        --sn-shadow-btn: 0 8px 24px rgba(91,78,255,0.35), 0 2px 8px rgba(0,0,0,0.12);
        --sn-surface: #ffffff;
        --sn-bg: #f4f5f8;
        --sn-border: rgba(0,0,0,0.07);
        --sn-text: #111827;
        --sn-subtext: #6b7280;
        --sn-bubble-out: var(--sn-accent);
        --sn-bubble-in: #ffffff;
      ...
      #sn-btn:hover {
        ...
        box-shadow: 0 16px 40px rgba(91,78,255,0.42), 0 4px 12px rgba(0,0,0,0.12);
      }
      ...
      #sn-panel {
        ...
        pointer-events: none;
        transition: opacity 0.28s cubic-bezier(0.22,1,0.36,1), transform 0.32s cubic-bezier(0.22,1,0.36,1), width 0.35s cubic-bezier(0.22,1,0.36,1), height 0.35s cubic-bezier(0.22,1,0.36,1), top 0.35s cubic-bezier(0.22,1,0.36,1), left 0.35s cubic-bezier(0.22,1,0.36,1), right 0.35s cubic-bezier(0.22,1,0.36,1), bottom 0.35s cubic-bezier(0.22,1,0.36,1), border-radius 0.35s ease;
      }
      #sn-panel.sn-open {
        opacity: 1;
        transform: translateY(0) scale(1);
      ...
      /* ── HEADER — dark gradient like MongoDB ── */
      #sn-header {
        background: linear-gradient(160deg, var(--sn-header-bg) 0%, var(--sn-header-bg2) 100%);
      ...
      #sn-avatar svg { width: 22px; height: 22px; }
      ...
      #sn-status-ring {
        ...
        border-radius: 50%;
        background: #6b7280;
        border: 2.5px solid var(--sn-header-bg);
        transition: background 0.3s ease;
      }
      #sn-status-ring.online { background: #22c55e; }
      #sn-status-ring.pulse {
      ...
      #sn-header-subtitle.sn-online::before {
        background: #22c55e;
        box-shadow: 0 0 0 2px rgba(34,197,94,0.3);
      }

      /* Header action buttons */
      #sn-expand-btn, #sn-close-btn {
        width: 30px;
        height: 30px;
        border-radius: 8px;
      ...
      #sn-expand-btn:hover, #sn-close-btn:hover {
        background: rgba(255,255,255,0.18);
      }
      #sn-expand-btn svg, #sn-close-btn svg {
        width: 14px;
        height: 14px;
      ...
      #sn-expand-btn { margin-right: 4px; }
      ...
      #sn-messages::-webkit-scrollbar { width: 4px; }
      #sn-messages::-webkit-scrollbar-track { background: transparent; }
      #sn-messages::-webkit-scrollbar-thumb { background: #d1d5db; border-radius: 99px; }

      /* AI bubble row wrapper — for avatar + bubble side by side */
      .sn-ai-row {
        display: flex;
      ...
      .sn-ai-avatar {
        ...
        flex-shrink: 0;
        background: var(--sn-accent-mid);
        border: 2px solid rgba(91,78,255,0.2);
        overflow: hidden;
        display: flex;
        align-items: center;
        justify-content: center;
      ...
      /* Group spacing */
      .sn-ai-row + .sn-ai-row { margin-top: 2px; }
      .sn-bubble.customer + .sn-ai-row,
      .sn-ai-row + .sn-bubble.customer,
      .sn-bubble.customer + .sn-bubble.customer { margin-top: 10px; }
      ...
      /* Customer bubbles animate themselves */
      .sn-bubble.customer {
        animation: snFadeUp 0.24s cubic-bezier(0.22,1,0.36,1) forwards;
      }
      .sn-bubble[dir="rtl"], .sn-bubble:dir(rtl) {
      ...
      /* AI bubble inside the row — no align-self needed */
      .sn-ai-row .sn-bubble.ai,
      .sn-ai-row .sn-bubble.human_agent {
        background: var(--sn-bubble-in);
        color: var(--sn-text);
      ...
      .sn-dot:nth-child(3) { animation-delay: 0.36s; }
      @keyframes snBounce {
        0%, 60%, 100% { transform: translateY(0); background: #c0c5d4; }
        30%           { transform: translateY(-5px); background: var(--sn-accent); }
      }
      ...
      #sn-input:disabled { cursor: not-allowed; color: #9ca3af; }
      ...
      #sn-send-btn:hover:not(:disabled) {
        background: var(--sn-accent-dark);
        transform: scale(1.06);
      }
      #sn-send-btn:active:not(:disabled) { transform: scale(0.93); }
      #sn-send-btn:disabled { opacity: 0.25; cursor: not-allowed; }
      #sn-send-btn svg {
        width: 16px;
        height: 16px;
        fill: none;
      ...
      @media (max-width: 480px) {
        ...
        #sn-btn { bottom: 18px; right: 18px; }
      }

      /* ── REDUCED MOTION ── */
      @media (prefers-reduced-motion: reduce) {
        #sn-panel, #sn-btn, .sn-bubble, .sn-dot, .sn-ai-row { animation: none; transition: none; }
      }
    `;
		document.head.appendChild(style);
	}
    ...
    `;
      ...
      <div id="sn-header">
        <div id="sn-avatar">
          <svg viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,0.85)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
            <path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"></path>
          </svg>
          <div id="sn-status-ring"></div>
        </div>
	...
	function applyWidgetConfig() {
		...
		// Pick up agent avatar from server config
		if (widgetConfig.agentAvatarUrl) {
			AI_AGENT_IMAGE = widgetConfig.agentAvatarUrl;
		}
	}
	...
	function updateStatus(state) {
		...
		if (state === "online") {
			...
			if (subtitle) {
				...
				subtitle.classList.add("sn-online");
			}
		} else {
			if (ring) ring.className = "";
			if (subtitle) {
				subtitle.textContent = "Connecting…";
				subtitle.classList.remove("sn-online");
			}
		}
	}
	...
	// AI agent avatar image URL — set this to your image URL
	var AI_AGENT_IMAGE = widgetConfig.agentAvatarUrl || null;

	function buildAiAvatarEl() {
		var avatarEl = document.createElement("div");
		avatarEl.className = "sn-ai-avatar";
		var imgSrc = AI_AGENT_IMAGE || (widgetConfig && widgetConfig.agentAvatarUrl) || null;
		if (imgSrc) {
			...
			img.onerror = function () {
				...
				fallback.setAttribute("class", "sn-ai-avatar-fallback");
				fallback.innerHTML = '<path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"></path>';
	...
	function appendMessage(role, content) {
		...
		// Wrap AI/human_agent in avatar row
		if (role === "ai" || role === "human_agent") {
			var row = document.createElement("div");
			row.className = "sn-ai-row";
			row.appendChild(buildAiAvatarEl());
