...
import {
	saveApiConfigService,
	getApiConfigService,
	verifyApiConfigService,
} from "src/services/businessApiConfig.service.js";
import AppError from "src/utils/appError.js";
import asyncHandler from "src/utils/asyncHandler.js";
import { ApiAuthType } from "generated/prisma/enums.js";

export const saveApiConfigController: RequestHandler = asyncHandler(
	async (req: AuthenticatedRequest, res) => {
		const organizationId = req.user?.organizationId;
		if (!organizationId) throw new AppError("Unauthorized", 401);

		const { baseUrl, authType, authValue, headerName } = req.body;
		...
		if (!Object.values(ApiAuthType).includes(authType)) {
			throw new AppError(
				`authType must be one of: ${Object.values(ApiAuthType).join(", ")}`,
				400,
			);
		}
...
export const getApiConfigController: RequestHandler = asyncHandler(
	async (req: AuthenticatedRequest, res) => {
		const organizationId = req.user?.organizationId;
		if (!organizationId) throw new AppError("Unauthorized", 401);
