...
import type { AuthenticatedRequest, CreateTicketBody, AssignTicketBody, ResolveTicketBody } from "../types/ticket.types.js";
...
export async function assignTicket(
  ...
  try {
   ...
   const ticketId = (req as Request<{ id: string }>).params.id;
    ...
    if (!assignedToId) {
      res.status(400).json({ success: false, message: "assignedToId is required." });
      return;
    }

    const ticket = await ticketService.assignTicket(orgId, ticketId, assignedToId);
...
export async function resolveTicket(
  ...
  try {
   ...
   const ticketId = (req as Request<{ id: string }>).params.id;
    ...
    const ticket = await ticketService.resolveTicket(orgId, ticketId, resolutionNote);
