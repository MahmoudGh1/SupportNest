...
export const widgetInitService = async ({ rawApiKey, origin, customerToken }: { rawApiKey: string; origin: string; customerToken?: string }) => {
	...
	const matchedKey = candidates.find((candidate) => candidate.keyHash === rawHash);
	...
	const widgetConf = typeof org.widgetConfig === "object" && org.widgetConfig ? (org.widgetConfig as Record<string, any>) : {};
