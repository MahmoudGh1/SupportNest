...
import { sendPasswordResetEmail, sendVerificationEmail } from "src/config/mailer.js";

export const registerService = async ({ email, password, firstName, lastName }: Omit<RegisterInput, "businessName" | "planId">) => {
...
export async function sendVerificationService(userId: string, email: string): Promise<void> {
...
export async function verifyEmailService(userId: string, code: string): Promise<void> {
	...
	if (record.expiresAt < new Date()) throw new AppError("Code has expired", 410);
...
export async function resetPasswordService(token: string, newPassword: string): Promise<void> {
	...
	if (record.used) throw new AppError("This reset link has already been used", 400);
	if (record.expiresAt < new Date()) throw new AppError("This reset link has expired", 410);
...
export const registerWithGoogleService = async (email: string, name?: string): Promise<{ userId: string; email: string; isNewUser: boolean }> => {
