...
function buildTool(toolDef: ToolDefinition, apiConfig: BusinessApiConfig): DynamicStructuredTool {
  ...
  for (const param of parameters) {
    ...
    let field: z.ZodTypeAny =
      param.type === "number" ? z.number() :
      param.type === "boolean" ? z.boolean() :
      z.string();
...
export async function runTier1Agent(
  ...
  const isAnonymous = !customer || customer.isAnonymous || !customer.externalId;
  ...
  if (isAnonymous) {
    ...
    const responseText = "To help you with your account I need to verify who you are first. Please log in using the link and come right back — I'll continue from where we left off.";

    return buildResponse(responseText, "NEEDS_AUTH", 0, 0, Date.now() - startTime, [], loginUrl);
  }

  // 4. Identified customer — build callable tools (exclude login tool)
  const callableToolDefs = apiConfig.toolDefinitions.filter(
    (t) => !(t.parameters as unknown as ToolParameter[])?.some((p) => p["x-supportnest-type"] === "login"),
  );
  ...
  const toolDescriptions = callableToolDefs.map((t) => `- ${t.name}: ${t.description}`).join("\n");
  ...
  let totalTokens = (firstResponse.usage_metadata as { total_tokens?: number } | undefined)?.total_tokens ?? 0;
  ...
  if (toolCalls.length > 0) {
    ...
    try {
      ...
      const parsed = JSON.parse(cleaned) as { agentText: string; confidenceScore: number };
    ...
    totalTokens += (secondResponse.usage_metadata as { total_tokens?: number } | undefined)?.total_tokens ?? 0;
  ...
  const action: Tier1ActionType = isResolved ? AgentAction.RESOLVED : AgentAction.ESCALATED_TO_TIER2;

  console.log(`[Tier1] tools: [${toolsUsed.join(", ")}] | confidence: ${confidenceScore} | action: ${action}`);
  ...
  return buildResponse(finalResponseText, action, confidenceScore, totalTokens, latencyMs, toolsUsed);
}