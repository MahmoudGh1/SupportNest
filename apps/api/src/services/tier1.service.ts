...
// Schema shared by both LLM calls so every code path returns the same shape.
const tier1OutputSchema = z.object({
	agentText: z.string(),
	confidenceScore: z.number(),
});
type Tier1Output = z.infer<typeof tier1OutputSchema>;
	...
	return new DynamicStructuredTool({
		...
		func: async (args: Record<string, unknown>) => {
			try {
				...
				// Guard against non-JSON responses (HTML error pages, redirects to a
				// login page, gateway timeouts, plain-text 500s, etc.) BEFORE calling
				// response.json(), which would throw on non-JSON bodies.
				const contentType = response.headers.get("content-type") ?? "";
				const rawBody = await response.text();

				if (!contentType.includes("application/json")) {
					console.error(`[Tier1 Tool Error] ${toolDef.name}: non-JSON response (status ${response.status}, content-type "${contentType}")`);
					return JSON.stringify({
						error: true,
						status: response.status,
				...
				try {
					...
					console.error(`[Tier1 Tool Error] ${toolDef.name}: failed to parse JSON body despite JSON content-type`, parseErr);
					return JSON.stringify({
						error: true,
						status: response.status,
						message: "External API response could not be parsed as JSON.",
					});
				}
...
function buildLoginUrl(loginTool: ToolDefinition, apiConfig: BusinessApiConfig, conversationId: string): string {
	const url = new URL(`${apiConfig.baseUrl}${loginTool.path}`);
	url.searchParams.set("state", conversationId);
	return url.toString();
}
...
/**
 * Safely extracts { agentText, confidenceScore } no matter what shape the
 * model returned. Handles:
 *  - withStructuredOutput's already-parsed object (the happy path)
 *  - a BaseMessage whose .content is a JSON string
 *  - a BaseMessage whose .content is JSON wrapped in ```json fences
 *  - a BaseMessage whose .content is an array of content blocks
 *  - genuinely unparsable content (falls back instead of throwing)
 */
function coerceToTier1Output(raw: unknown, fallbackText = "I don't have access to that information."): Tier1Output {
	// Already a plain object matching the schema (withStructuredOutput happy path).
	if (raw && typeof raw === "object" && !("content" in (raw as object))) {
		const parsed = tier1OutputSchema.safeParse(raw);
		if (parsed.success) return parsed.data;
	}

	// BaseMessage-like: pull out .content and try to parse it as JSON.
	...
	if (text) {
		...
		try {
			...
			const parsed = tier1OutputSchema.safeParse(parsedJson);
			if (parsed.success) return parsed.data;
		} catch {
			// Not JSON at all (plain prose, HTML, etc.) — fall through to fallback below.
		}

		// Model returned plain text instead of JSON. Don't discard it silently —
		// surface it as the agentText with a reduced confidence score rather than
		// returning an empty string to the caller.
		if (cleaned.length > 0) {
			return { agentText: cleaned, confidenceScore: 0.3 };
		}
	}

	return { agentText: fallbackText, confidenceScore: 0 };
}
...
export async function runTier1Agent(context: PipelineContext): Promise<TierResponse> {
	...
	let firstResponse;
	try {
		firstResponse = await modelWithTools.invoke([
			new SystemMessage(`
You are a customer support agent with access to business APIs.
...
- Always base your response on real tool output.
    ...
    `),
		...
		// Provider/network failure on the first call — return a clean, typed
		// empty result instead of letting this bubble up as an unhandled
		// rejection (which is what tends to surface as a raw HTML 500 page
		// further up the stack).
		console.error("[Tier1] First LLM call failed:", err);
		return {
			tier: MessageTier.TIER1,
			responseText: "",
			agentLog: { confidenceScore: 0, tokensUsed: 0 },
	...
	// 5. No tool selected — model couldn't match any tool to the question.
	// IMPORTANT: previously this always returned responseText: "" and threw
	// away whatever the model said, even if it had followed instructions and
	// returned the "no tool matches" JSON. An empty responseText is what was
	// likely causing a downstream renderer to fall back to a generic error
	// page. We now coerce whatever the model said into a real Tier1Output.
	if (toolCalls.length === 0) {
		const { agentText, confidenceScore } = coerceToTier1Output(firstResponse);
		await appendToMemory(conversationId, latestMessage, agentText);
		return {
			tier: MessageTier.TIER1,
			responseText: agentText,
	...
	try {
		secondResponse = await jsonModel.invoke([
			new SystemMessage(`
You are a customer support agent. Use the tool results below to write a reply.
...
${toolResults.join("\n\n")}
    `),
		...
		// withStructuredOutput can still throw if the provider returns something
		// it can't coerce to the schema. Don't let this become an unhandled
		// rejection — fall back to a safe, typed response.
		console.error("[Tier1] Second LLM call failed:", err);
		const totalTokens = firstTokens;
		await appendToMemory(conversationId, latestMessage, "I'm having trouble pulling that up right now — could you try again in a moment?");
		return {
			tier: MessageTier.TIER1,
			responseText: "I'm having trouble pulling that up right now — could you try again in a moment?",
			agentLog: {
				confidenceScore: 0,
				tokensUsed: totalTokens,
			},
	...
	const totalTokens = firstTokens + ((secondResponse as { usage_metadata?: { total_tokens?: number } })?.usage_metadata?.total_tokens ?? 0);

	// 8. Parse response safely — coerceToTier1Output handles every shape
	// withStructuredOutput might hand back, plus the previous edge cases that
	// used to throw away the model's text and fall back to an empty string.
	const { agentText, confidenceScore } = coerceToTier1Output(secondResponse, "I'm having trouble pulling that up right now — could you try again in a moment?");
