...
async function buildConversationStats(
  ...
  const [total, active, escalated, closed] = await Promise.all([
    ...
    prisma.conversation.count({ where: { ...where, conversationStatus: 'ACTIVE' } }),
    prisma.conversation.count({ where: { ...where, conversationStatus: 'ESCALATED' } }),
    prisma.conversation.count({ where: { ...where, conversationStatus: 'CLOSED' } }),
  ]);
...
async function buildCsatSummary(
  ...
  const [agg, distribution] = await Promise.all([
    prisma.csatRating.aggregate({ where, _avg: { score: true }, _count: { id: true } }),
    prisma.csatRating.groupBy({ by: ['score'], where, _count: { id: true }, orderBy: { score: 'asc' } }),
  ]);
...
async function buildRecentEscalations(
  ...
  return tickets.map((t) => ({
    ...
    assigned_to: t.assignedTo
      ? { id: t.assignedTo.id, full_name: `${t.assignedTo.firstName} ${t.assignedTo.lastName}` }
      : null,
...
export async function listOrganizations(opts: {
  ...
  const [activeConvCounts, openTicketCounts, escalatedTicketCounts, resolvedTicketCounts] =
    await Promise.all([
  ...
  const byOrgId = <T extends { organizationId: string; _count: { id: number } }>(arr: T[]) =>
